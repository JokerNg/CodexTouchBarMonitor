...
final class CodexAppServerClient {
    ...
    private let queue = DispatchQueue(label: "CodexTouchBarMonitor.CodexAppServerClient")
    ...
    func readUsage(completion: @escaping (Result<GetAccountUsageResponse, Error>) -> Void) {
        request(method: "account/usage/read", params: nil) { result in
            ...
            case .success(let value):
                do {
                    ...
                    let data = try JSONSerialization.data(withJSONObject: value)
                    let response = try JSONDecoder().decode(GetAccountUsageResponse.self, from: data)
                    completion(.success(response))
                } catch {
                    completion(.failure(error))
                }
